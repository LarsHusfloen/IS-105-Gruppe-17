...
int main()
{
	...
	if (pid == 0) /* checks if the pid is equal to 0, if true it's the child process */
	{
		...
		for (i = 0; i < strlen(read_msg); i++) /* Char is represented with int,
		 										* ASCII to convert from upper to lowercase and vice versa */
		{
			if ((int)read_msg[i] >= 65 && (int)read_msg[i] <= 90) /* Checks if the letter is uppercase */
			{
				write_msg[i] = (int)read_msg[i] + 32; /* Upper and lowercase are 32 apart in the ASCII table */
			}
			else if ((int)read_msg[i] >= 97 && (int)read_msg[i] <= 122) /* Checks if the letter is lowercase */
			{
				write_msg[i] = (int)read_msg[i] - 32; /* Upper and lowercase are 32 apart in the ASCII table */
			}
		}
